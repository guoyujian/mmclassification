...
class ImbalancedDatasetSampler(torch.utils.data.sampler.Sampler):
    ...
    def __init__(
        ...
        # print(f'df.labels: {len(df["label"])}  {df["label"]}')
        assert len(df["label"]) == len(dataset)
    ...
    def _get_labels(self, dataset):
        if self.callback_get_label:
            ...
            # print(f'list: {dataset.get_gt_labels().tolist()}')
            return dataset.get_gt_labels().tolist()
